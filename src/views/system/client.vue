...
export default {
  ...
  methods: {
    ...
    getClientApi(row) {
      ...
      clientApiGet(this.clientApi.clientAppId, this.clientApi).then(response => {
        const data = response.data
        if (is200(data.code)) {
    ...
    getClientIp(clientAppId) {
      clientIpGet(clientAppId).then(response => {
        const data = response.data
        if (is200(data.code)) {
    ...
    handleClientApi(row) {
      ...
      this.fromData = []
      this.toData = []
      this.getClientIp(row.clientAppId)
      this.clientApi.clientAppId = row.clientAppId
    },
    handleDeleteIp(row) {
      this.$confirm('删除IP将会删除与IP关联的所有API, 是否继续?', '提示', {
        ...
        this.clientApi.ip = row.ip
        clientIpDel(this.clientApi.clientAppId, this.clientApi).then(response => {
          const data = response.data
          if (is200(data.code)) {
            this.$notify({
              title: '成功',
            ...
            this.getClientIp(this.clientApi.clientAppId)
            this.fromData = []
            this.toData = []
          } else {
            this.$notify({
    ...
    handleForbid(row) {
      // todo 禁用？？
      clientDelete(row.clientAppId).then(response => {
        const data = response.data
        if (is200(data.code)) {
    ...
    update() {
      clientUpdate(this.client.clientAppId, this.client).then(response => {
        const data = response.data
        if (is200(data.code)) {
          this.dialogFormVisible = false
          this.$notify({
